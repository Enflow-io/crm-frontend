...
function MyApp({ Component, pageProps }: AppPropsWithLayout) {
    ...
    const socketPath = process.env.NEXT_PUBLIC_API_HOST_PROD
        ? process.env.NEXT_PUBLIC_API_HOST_PROD.replace("/api", "")
        : "http://localhost:3010";
    ...
    useEffect(() => {
        ...
        function showCalendarNotification(payload: any) {
            // @ts-ignore
            const userMessages = payload.filter(
            ...
            if (userMessages.length > 0) {
                for (const message of userMessages) {
                    const config: ArgsProps = {
                        description: `${message.description} в ${new Date(
                            message.date
                        )
                            .toTimeString()
                            .slice(0, 5)}`,
